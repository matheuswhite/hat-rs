...
impl Future for WorkQueueNotifierFuture {
    type Output = ();

    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let Ok(mut state) = self.state.lock() else {
            panic!("Cannot get work queue notifier future mutex");
        };
        ...
        match &mut *state {
            WorkQueueState::None => {
                *state = WorkQueueState::Waiting(cx.waker().clone());
                ...
                unsafe {
                    hat_zbus_notify_work_queue(self.channel_reference.as_ptr(),
                                               hat_zbus_work_queue_notify_done,
                                               state_ptr);
                }

                Poll::Pending
            }
            WorkQueueState::Waiting(_) => panic!("The \"Waiting\" state must be unreachable at the work queue notifier future poll"),
...
#[no_mangle]
pub extern "C" fn hat_zbus_work_queue_notify_done(state: *const ()) {
    let state = unsafe {
        Arc::from_raw(state as *const BlockingMutex<WorkQueueState>)
    };
    let mut state = state.lock()
        .hat_expect("Cannot lock at 'hat_zbus_work_queue_notify_done'");
...
extern "C" {
    fn hat_zbus_notify_work_queue(chan: *const struct_zbus_channel,
