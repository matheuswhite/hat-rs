...
impl<T: Default> Future for WorkQueueWaiterFuture<T> {
    type Output = T;

    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let mut data = self.data.lock().hat_expect("Cannot lock data mutex");
        let Some(data_ptr) = data.as_mut() else {
            panic!("WorkQueueWaiter's data cannot be 'None'")
        };
        ...
        match &mut *state {
            WorkQueueState::None => {
                *state = WorkQueueState::Waiting(cx.waker().clone());
                ...
                unsafe {
                    hat_zbus_wait_work_queue(self.channel_reference.as_ptr(),
                                             data_ptr as *const T as *mut T as *mut (),
                                             hat_zbus_work_queue_wait_done,
                                             state_ptr);
                }
            ...
            WorkQueueState::Completed => {
                match data.take() {
                    None => panic!("WorkQueueWaiter's data cannot be 'None' at 'Completed' state"),
...
#[no_mangle]
pub extern "C" fn hat_zbus_work_queue_wait_done(state: *const ()) {
    let state = unsafe {
        Arc::from_raw(state as *const BlockingMutex<WorkQueueState>)
    };
    let mut state = state.lock()
        .hat_expect("Cannot lock at 'hat_zbus_work_queue_wait_done'");
...
extern "C" {
    fn hat_zbus_wait_work_queue(chan: *const struct_zbus_channel, msg: *mut (),
